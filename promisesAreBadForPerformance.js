...
async function main() {
    ...
    console.log(`\x1b[32mRecursive: Promises are ${(fibRecPromTime / fibRecNormTime).toFixed(2)} times slower than normal.\x1b[0m\n`);

    const fibNormTime = await run(fibNormal);
    const fibPromTime = await run(fibPromise, true);
    console.log(`\x1b[32mNon-Recursive: Promises are ${(fibPromTime / fibNormTime).toFixed(2)} times slower than normal.\x1b[0m\n`);

    const fibPromTime2 = await run(fibPromise, true);
    const fibPromDirectTime = await run(fibPromDirect, true);
    console.log(`\x1b[32mNon-Recursive: Promises are ${(fibPromDirectTime / fibPromTime2).toFixed(2)} times slower than normal.\x1b[0m`);
}
